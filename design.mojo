...
- Stack: 7mm x 7mm x 18mm, stroke ~24um, blocked force ~1900N
...
comptime PIEZO_STROKE: Float64 = 24.0e-6  # m - nominal bipolar stroke
...
comptime PIEZO_CLEARANCE: Float64 = 2.0e-3  # m, total clearance around piezo (1mm each side)
comptime L0_BASE: Float64 = PIEZO_WIDTH + PIEZO_CLEARANCE  # m, minimum vertical input port height
...
comptime D_BASE_MIN: Float64 = 9.0e-3  # m
comptime D_BASE_MAX: Float64 = 9.0e-3  # m
comptime D_BASE_STEP: Float64 = 1.0e-3  # m

# Limb dimensions
comptime L_TOTAL: Float64 = 15.0e-3  # m, total limb length (for rhombic)
comptime L_FLEX: Float64 = 2.0e-3  # m, flexure hinge length
comptime L_LINK: Float64 = 3.0e-3  # m, rigid link length (for parallel/aligned)
comptime H_LINK: Float64 = 5.0e-3  # m, link thickness (essentially rigid)

# Output port dimensions
comptime L_OUT: Float64 = 4.0e-3  # m, output port length
comptime H_OUT: Float64 = 5.0e-3  # m, output port height
...
comptime THETA_MIN: Float64 = -10.0  # degrees
comptime THETA_MAX: Float64 = -6.0  # degrees (avoid singularity near 0)
comptime THETA_STEPS: Int = 100

comptime H_FLEX_MIN: Float64 = 0.4e-3  # m
comptime H_FLEX_MAX: Float64 = 0.5e-3  # m
...
fn get_design_output_mass() -> OutputMass:
    """Output mass: Small 4x4x8 mm aluminum block (~0.5g)."""
    return make_output_mass_from_block(4.0e-3, 4.0e-3, 8.0e-3, 2770.0)
...
fn make_rhombic_geometry(
    theta_deg: Float64, h_flex: Float64, d: Float64
) -> GeometricParameters:
    """
...
fn compute_H_from_theta(theta_deg: Float64) -> Float64:
    """Compute vertical offset H from theta for parallel amplifier."""
    var theta_rad = theta_deg * pi / 180.0
    return L_LINK * tan(-theta_rad)  # Positive H for negative theta


fn make_parallel_geometry(
    theta_deg: Float64, h_flex: Float64, d: Float64
) -> GeometricParameters:
    """
    ...
    Input port L_0 scales with H per paper recommendation:
    L_0 = PIEZO_WIDTH + clearance + H (keep input port as short as possible)
    """
    var H = compute_H_from_theta(theta_deg)

    # Input port height scales with H to accommodate the offset
    var L_0 = L0_BASE + H
    var input_port = InputPortParams(L0=L_0, h0=H0_BASE)
...
fn run_2d_sweep(
    ...
    var output_mass = make_output_mass_from_block(L_OUT, H_OUT, d, mat.rho)
    ...
    for i in range(THETA_STEPS):
        ...
        for j in range(H_FLEX_STEPS):
            ...
            var analysis = compute_results(
                mat, geom, output_mass, input_mass=0.0, single_sided=True
            )
            ...
            # Store H for parallel amplifier (0 for others)
            if structure == "parallel":
                result.H_mm = compute_H_from_theta(theta_deg) * 1000.0
            else:
                result.H_mm = 0.0
            ...
            result.preload = estimate_preload(
                theta_deg, analysis.input_stiffness
            )

            # Check constraints
            result.meets_constraints = (
                result.ratio >= RATIO_MIN
                and result.stiffness <= STIFFNESS_MAX
                and result.preload >= PRELOAD_MIN
            )
...
fn plot_ratio_stiffness_contours(
    ...
    var theta_vals = np.abs(np.linspace(THETA_MIN, THETA_MAX, THETA_STEPS))
    var h_vals = np.linspace(
        H_FLEX_MIN * 1000.0, H_FLEX_MAX * 1000.0, H_FLEX_STEPS
    )

    # Build H values array for parallel amplifier (convert theta to H)
    var H_vals_list = builtins.list()
    for i in range(THETA_STEPS):
        var theta_deg = THETA_MIN + Float64(i) * (
            THETA_MAX - THETA_MIN
        ) / Float64(THETA_STEPS - 1)
        H_vals_list.append(compute_H_from_theta(theta_deg) * 1000.0)  # mm
    var H_offset_vals = np.array(H_vals_list)
    ...
    # Helper function to create and save a figure
    fn save_mechanics_figure(
        x_vals: PythonObject,
        x_label: String,
        ...
        var ax1 = axes[0]
        var c1 = ax1.contourf(x_vals, h_vals, ratio_grid.T, 20)
        ...
        var ax2 = axes[1]
        var c2 = ax2.contourf(x_vals, h_vals, stiff_grid.T, 20)
    ...
    # Save theta version (always)
    save_mechanics_figure(theta_vals, String("theta (deg)"), String("_theta"))

    # Save H version for parallel amplifier
    if structure == "parallel":
        save_mechanics_figure(H_offset_vals, String("H (mm)"), String("_H"))
...
fn plot_frequency_contours(
    ...
    fn save_frequency_figure(
        ...
        fig.suptitle(
            ...
            + " (d="
            + d_str
            + "mm) - Natural Frequencies",
            fontsize=14,
        )

        # Subplot (a): f1
        var ax1 = axes[0]
        var c1 = ax1.contourf(x_vals, h_vals, f1_grid.T, 20)
        plt.colorbar(c1, ax=ax1)
        ax1.set_xlabel(x_label)
        ax1.set_ylabel(String("h_flex (mm)"))
        ...
        # Subplot (b): f2
        var ax2 = axes[1]
        var c2 = ax2.contourf(x_vals, h_vals, f2_grid.T, 20)
        plt.colorbar(c2, ax=ax2)
        ax2.set_xlabel(x_label)
        ax2.set_ylabel(String("h_flex (mm)"))
        ...
        # Subplot (c): f3
        var ax3 = axes[2]
        var c3 = ax3.contourf(x_vals, h_vals, f3_grid.T, 20)
        plt.colorbar(c3, ax=ax3)
        ax3.set_xlabel(x_label)
        ax3.set_ylabel(String("h_flex (mm)"))
    ...
    # Save theta version (always)
    save_frequency_figure(theta_vals, String("theta (deg)"), String("_theta"))

    # Save H version for parallel amplifier
    if structure == "parallel":
        save_frequency_figure(H_offset_vals, String("H (mm)"), String("_H"))
...
fn print_summary_table(
    ...
    print("\n" + "=" * 90)
    print(
        "TOP CONFIGURATIONS (meeting constraints: ratio >=",
        RATIO_MIN,
        ", k_in <=",
        STIFFNESS_MAX,
        "N/um, preload >=",
    ...
    print("=" * 90)
    print(
        "Rank | Structure | Material    | theta(deg) | h(mm) | Ratio | k_in  |"
        " f1(Hz) | Preload(N)"
    )
    print("-" * 90)
    ...
    while d <= D_BASE_MAX + 1e-6:
        ...
        for s_idx in range(len(structures)):
            ...
            for m_idx in range(len(mat_names)):
                ...
                try:
                    plot_ratio_stiffness_contours(
                        results, structure, mat_name, d_mm
                    )
                ...
                var best = find_best_configurations(
                    results, structure, mat_name, top_n=3
                )
